<template>
  <div>
    ...
    >
      ...
      <div v-if="!multiple" ref="input" class="sp-base sp-input">
        <span v-html="results" />
        <span v-show="picked.length === 0" class="sp-placeholder">{{
          placeholderString
        }}</span>
      </div>
      <div v-if="multiple" ref="input" class="sp-base sp-inputs">
        <span v-show="picked.length === 0" class="sp-placeholder">{{
          placeholderString
        }}</span>
        <span
          v-for="(sel, index) in picked"
          :key="index"
          class="sp-selected-tag"
        >
          <span v-html="renderCell(sel)" />
          <span v-show="!disabled" @click="remove(index)">
            <i class="sp-iconfont sp-icon-close" />
          </span>
        </span>
      </div>
      <div
    ...
    <v-dropdown ref="dropdown" @show-change="dropdownVisible">
      <!-- message bar -->
      <div v-if="message" class="sp-message">
        <i class="sp-iconfont sp-icon-warning" />
        <span v-html="message" />
      </div>
      <!-- header bar -->
      <div v-show="!message" class="sp-header">
        ...
        <button
          ...
          @click="pickPage(true)"
        >
          <i class="sp-iconfont sp-icon-select-all" />
        </button>
        <button
          v-if="multiple"
          type="button"
        ...
        >
          <i class="sp-iconfont sp-icon-unselect-all" />
        </button>
        <button
          type="button"
          ...
          @click="remove"
        >
          <i class="sp-iconfont sp-icon-clear" />
        </button>
        <button
          type="button"
      ...
      <!-- search bar -->
      <div v-show="!message" class="sp-search">
        <input
          ref="search"
          v-model="search"
          type="text"
      ...
      <!-- content list bar -->
      <div v-show="!message" ref="list" class="sp-result-area">
        ...
        >
          <li
            ...
            :class="{
              ...
              'sp-selected':
                picked.findIndex((val) => val[keyField] === row[keyField]) !==
                -1,
            }"
        ...
        <!-- no result message -->
        <div v-if="!list.length" class="sp-message" v-text="i18n.not_found" />
      </div>

      <!-- pagination bar -->
      <div v-show="!message" v-if="pagination" class="sp-pagination">
        ...
        <ul ref="page">
          ...
          >
            <a href="javascript:void(0);" @click="switchPage('last')">
              <i class="sp-iconfont sp-icon-last" />
            </a>
          </li>
          <li
            :class="{ 'sp-disabled': pageNumber === totalPage }"
          ...
          >
            <a href="javascript:void(0);" @click="switchPage('next')">
              <i class="sp-iconfont sp-icon-next" />
            </a>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    open() {
      console.log(this.disabled, 'this.disabled', this.show)
    ...
    pickPage(check) {
      ...
      for (const row of this.list) {
        if (
          ...
          this.maxSelectLimit &&
          this.picked.length < this.maxSelectLimit &&
          check
        ) {
    ...
    sortList() {
      // console.log(this.data, Array.isArray(this.data), this.sort)
